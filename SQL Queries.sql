...
JOIN grommet_product_to_keyword ON grommet_product_to_keyword.keyword_id = grommet_product_keywords.id
JOIN grommet_products ON grommet_products.id = grommet_product_to_keyword.product_id
JOIN grommet_product_categories ON grommet_products.id = grommet_product_categories.product_id
JOIN grommet_gifts_categories ON grommet_gifts_categories.id = grommet_product_categories.product_category_id

WHERE grommet_gifts_categories.sub_category='Beauty & Personal Care' OR  grommet_product_keywords.keyword='Aromatherapy' AND grommet_products.is_sold_out = false GROUP BY grommet_products.id;
