...
INNER JOIN grommet_product_categories ON grommet_gifts_categories.id = grommet_product_categories.product_category_id 
INNER JOIN grommet_product_keywords ON grommet_gifts_categories.id = grommet_product_keywords.id 
INNER JOIN grommet_product_to_keyword ON grommet_product_keywords.id = grommet_product_to_keyword.keyword_id 
INNER JOIN grommet_products ON grommet_product_categories.product_id = grommet_products.id AND grommet_product_to_keyword.product_id = grommet_products.id
WHERE grommet_gifts_categories.sub_category='Beauty & Personal Care' AND grommet_products.is_sold_out = false OR grommet_product_keywords.keyword='Aromatherapy' ;
